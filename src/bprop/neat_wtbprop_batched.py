...
from src.bprop.bprop_utils import grad_fn, sgd_update
...
def evaluate_population_classification(genomes: list, pop_size: int | None = None) -> np.ndarray:
    """
    Evaluates the population of genomes on the classification task.
    Weights are fine-tuned using backpropagation before evaluation.

    Args:
        genomes (list): _description_
        pop_size (int | None, optional): _description_. Defaults to None.

    Returns:
        np.ndarray: _description_
    """

    N_MAX = max(len(g.nodes) for g in genomes)
    params_batch = genomes_to_params_batch(genomes, OBS_DIM, ACT_DIM)
    weights_batch = params_batch["conn_weight"]
    
    # Extract STATIC parts (non-trainable) for use in the grad function
    # These must be batched (P, ...) for jax.vmap
    static_params_batch = {k: v for k, v in params_batch.items() 
                           if k not in ["conn_weight"]}
    
    # Vmap the gradient calculation over the Population (P)
    # The gradient is computed w.r.t. arg0 (conn_weight)
    # All other arguments (X, Y, statics) are shared (None) or batched (0)
    # We need a new vmap for the specific arguments of compute_loss_for_genome
    grad_fn_batched = jax.vmap(
        grad_fn, 
        in_axes=(0, 0, None, None, 0, None, None), # 0: weights, enabled; None: X, Y; 0: statics
        out_axes=0
    )
    
    LR = 0.01
    BP_STEPS = 5 # A few steps, do not want to overoptimise a given network topology
    BATCH_SIZE = 32 
    DATA_SIZE = X_DATA.shape[0]

    # random indices for all BP steps
    rng_seed = int(time.time() * 1e6) & 0xFFFFFFFF
    key = jax.random.PRNGKey(rng_seed)
    key, subkey = jax.random.split(key) # Assuming a JAX key is available
    indices = jax.random.randint(subkey, 
                            shape=(BP_STEPS, BATCH_SIZE), 
                            minval=0, 
                            maxval=DATA_SIZE)
    for step in range(BP_STEPS):
        minibatch_indices = indices[step]
        X_BATCH = X_DATA[minibatch_indices]
        Y_BATCH = Y_DATA[minibatch_indices]
        gradients_batch = grad_fn_batched(
            weights_batch,
            params_batch["conn_enabled"],
            X_BATCH, 
            Y_BATCH,
            static_params_batch,
            ACT_DIM,
        ...
        # Weights updated for ALL P genomes simultaneously
        sgd_update_batched = jax.vmap(sgd_update, in_axes=(0, 0, None))
        weights_batch = sgd_update_batched(weights_batch, gradients_batch, LR)
    
    # Update the parameters dictionary with the optimized weights
    params_batch["conn_weight"] = weights_batch
    
    # 2. Calculate Final Fitness (using the optimized params_batch)
...
def train_neat_on_classification(generations: int = 200, pop_size: int = 200):
    ...
    log_dir = Path('./log/classification_brpropwts')
